...
local Radius = 512
local Damage = 200
local KnockDownDoors = true

if SERVER then
	function ENT:Initialize()
		self.BaseClass.Initialize(self)
		self:SetPlanted(false)
		self:SetCountdown(Timer)
	end
		...
		if IsValid(caller) and caller:IsPlayer() then
			if self:GetPlanted() then
				...
				timer.Simple(Timer + 1, function()
					if not IsValid(self) then return end
					util.BlastDamage(self, self, self:GetPos(), Radius, Damage)
					local boom = EffectData()
					boom:SetOrigin(self:GetPos())
					util.Effect("HelicopterMegaBomb", boom)
					self:EmitSound(Sound("phx/explode00.wav"))
					if KnockDownDoors then
